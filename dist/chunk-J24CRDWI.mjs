...
// src/clickhouse.ts
import * as SQLParser2 from "node-sql-parser";
...
var hash = (input) => Math.abs(hashCode(`${input}`));
var paramHash = (str) => {
      ...
      if (prefixWildcard || suffixWildcard) {
        ...
        if (hasSeperators) {
          ...
          return `(${isNegatedField ? "NOT (" : ""}${[
            ...tokens.map(
              (token) => SqlString.format(`${hasTokenFn}(?, ?)`, [
                SqlString.raw(column ?? ""),
                token
              ])
            ),
