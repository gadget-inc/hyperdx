...
var _chunkBFPDEFYAjs = require('./chunk-BFPDEFYA.js');
...
var Metadata = class {
  ...
  async queryTableMetadata({
    ...
    return cache.getOrFetch(`${database}.${table}.metadata`, async () => {
      const sql = _chunkBFPDEFYAjs.chSql`SELECT * FROM system.tables where database = ${{ String: database }} AND name = ${{ String: table }}`;
  ...
  async getColumns({
    ...
    return this.cache.getOrFetch(
      ...
      async () => {
        const sql = _chunkBFPDEFYAjs.chSql`DESCRIBE ${_chunkBFPDEFYAjs.tableExpr.call(void 0, { database: databaseName, table: tableName })}`;
  ...
  async getMapKeys({
    ...
    const where = metricName ? _chunkBFPDEFYAjs.chSql`WHERE MetricName=${{ String: metricName }}` : "";
    let sql;
    if (strategy === "groupUniqArrayArray") {
      sql = _chunkBFPDEFYAjs.chSql`SELECT groupUniqArrayArray(${{ Int32: maxKeys }})(${{
        Identifier: column
      }}) as keysArr
      FROM ${_chunkBFPDEFYAjs.tableExpr.call(void 0, { database: databaseName, table: tableName })} ${where}`;
    } else {
      sql = _chunkBFPDEFYAjs.chSql`SELECT DISTINCT lowCardinalityKeys(arrayJoin(${{
        Identifier: column
      }}.keys)) as key
      FROM ${_chunkBFPDEFYAjs.tableExpr.call(void 0, { database: databaseName, table: tableName })} ${where}
      LIMIT ${{
  ...
  async getMapValues({
    ...
    const sql = key ? _chunkBFPDEFYAjs.chSql`
      SELECT DISTINCT ${{
      Identifier: column
    }}[${{ String: key }}] as value
      FROM ${_chunkBFPDEFYAjs.tableExpr.call(void 0, { database: databaseName, table: tableName })}
      WHERE value != ''
      LIMIT ${{
      Int32: maxValues
    }}
    ` : _chunkBFPDEFYAjs.chSql`
      SELECT DISTINCT ${{
      Identifier: column
    }} as value
      FROM ${_chunkBFPDEFYAjs.tableExpr.call(void 0, { database: databaseName, table: tableName })}
  ...
  async getAllFields({
    ...
    for (const c of columns) {
      fields.push({
        path: [c.name],
        type: c.type,
        jsType: _chunkBFPDEFYAjs.convertCHDataTypeToJSType.call(void 0, c.type)
      });
    }
    const mapColumns = _nullishCoalesce(_chunkBFPDEFYAjs.filterColumnMetaByType.call(void 0, columns, ["map" /* Map */]), () => ( []));
    await Promise.all(
      mapColumns.map(async (column) => {
        ...
        for (const key of keys) {
          fields.push({
            ...
            type: chType,
            jsType: _chunkBFPDEFYAjs.convertCHDataTypeToJSType.call(void 0, chType)
          });
        }
      })
    );
  ...
  async getKeyValues({
    ...
    return this.cache.getOrFetch(
      ...
      async () => {
        const sql = await _chunkBFPDEFYAjs.renderChartConfig.call(void 0, 
          {
            ...chartConfig,
