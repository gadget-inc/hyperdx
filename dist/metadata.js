...
const DEFAULT_MAX_ROWS_TO_READ = 3e6;
class MetadataCache {constructor() { MetadataCache.prototype.__init.call(this);MetadataCache.prototype.__init2.call(this); }
...
class Metadata {
  ...
  async queryTableMetadata({
    ...
    return cache.getOrFetch(`${database}.${table}.metadata`, async () => {
      const sql = _clickhouse.chSql`SELECT * FROM system.tables where database = ${{ String: database }} AND name = ${{ String: table }}`;
  ...
  async getColumns({
    ...
    return this.cache.getOrFetch(
      ...
      async () => {
        const sql = _clickhouse.chSql`DESCRIBE ${_clickhouse.tableExpr.call(void 0, { database: databaseName, table: tableName })}`;
  ...
  async getMapKeys({
    ...
    const where = metricName ? _clickhouse.chSql`WHERE MetricName=${{ String: metricName }}` : "";
    let sql;
    if (strategy === "groupUniqArrayArray") {
      sql = _clickhouse.chSql`SELECT groupUniqArrayArray(${{ Int32: maxKeys }})(${{
        Identifier: column
      }}) as keysArr
      FROM ${_clickhouse.tableExpr.call(void 0, { database: databaseName, table: tableName })} ${where}`;
    } else {
      sql = _clickhouse.chSql`SELECT DISTINCT lowCardinalityKeys(arrayJoin(${{
        Identifier: column
      }}.keys)) as key
      FROM ${_clickhouse.tableExpr.call(void 0, { database: databaseName, table: tableName })} ${where}
      LIMIT ${{
  ...
  async getMapValues({
    ...
    const sql = key ? _clickhouse.chSql`
      SELECT DISTINCT ${{
      Identifier: column
    }}[${{ String: key }}] as value
      FROM ${_clickhouse.tableExpr.call(void 0, { database: databaseName, table: tableName })}
      WHERE value != ''
      LIMIT ${{
      Int32: maxValues
    }}
  ...
  async getAllFields({
    ...
    for (const c of columns) {
      fields.push({
        path: [c.name],
        type: c.type,
        jsType: _clickhouse.convertCHDataTypeToJSType.call(void 0, c.type)
      });
    }
    const mapColumns = _nullishCoalesce(_clickhouse.filterColumnMetaByType.call(void 0, columns, [_clickhouse.JSDataType.Map]), () => ( []));
    await Promise.all(
      mapColumns.map(async (column) => {
        ...
        for (const key of keys) {
          fields.push({
            ...
            type: chType,
            jsType: _clickhouse.convertCHDataTypeToJSType.call(void 0, chType)
          });
        }
      })
    );
  ...
  async getKeyValues({
    ...
    return this.cache.getOrFetch(
      ...
      async () => {
        const sql = await _renderChartConfig.renderChartConfig.call(void 0, 
          {
            ...chartConfig,
...
const getMetadata = (clickhouseClient) => new Metadata(clickhouseClient, __LOCAL_CACHE__);
