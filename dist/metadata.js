...
var _chunkFUSDVFROjs = require('./chunk-FUSDVFRO.js');
require('./chunk-TBQHB7JC.js');
...
var Metadata = class {
  ...
  async queryTableMetadata({
    ...
    return cache.getOrFetch(`${database}.${table}.metadata`, async () => {
      const sql = _chunkFUSDVFROjs.chSql`SELECT * FROM system.tables where database = ${{ String: database }} AND name = ${{ String: table }}`;
  ...
  async getColumns({
    ...
    return this.cache.getOrFetch(
      ...
      async () => {
        const sql = _chunkFUSDVFROjs.chSql`DESCRIBE ${_chunkFUSDVFROjs.tableExpr.call(void 0, { database: databaseName, table: tableName })}`;
  ...
  async getMapKeys({
    ...
    const where = metricName ? _chunkFUSDVFROjs.chSql`WHERE MetricName=${{ String: metricName }}` : "";
    let sql;
    if (strategy === "groupUniqArrayArray") {
      sql = _chunkFUSDVFROjs.chSql`SELECT groupUniqArrayArray(${{ Int32: maxKeys }})(${{
        Identifier: column
      }}) as keysArr
      FROM ${_chunkFUSDVFROjs.tableExpr.call(void 0, { database: databaseName, table: tableName })} ${where}`;
    } else {
      sql = _chunkFUSDVFROjs.chSql`SELECT DISTINCT lowCardinalityKeys(arrayJoin(${{
        Identifier: column
      }}.keys)) as key
      FROM ${_chunkFUSDVFROjs.tableExpr.call(void 0, { database: databaseName, table: tableName })} ${where}
      LIMIT ${{
  ...
  async getMapValues({
    ...
    const sql = key ? _chunkFUSDVFROjs.chSql`
      SELECT DISTINCT ${{
      Identifier: column
    }}[${{ String: key }}] as value
      FROM ${_chunkFUSDVFROjs.tableExpr.call(void 0, { database: databaseName, table: tableName })}
      WHERE value != ''
      LIMIT ${{
      Int32: maxValues
    }}
    ` : _chunkFUSDVFROjs.chSql`
      SELECT DISTINCT ${{
      Identifier: column
    }} as value
      FROM ${_chunkFUSDVFROjs.tableExpr.call(void 0, { database: databaseName, table: tableName })}
  ...
  async getAllFields({
    ...
    for (const c of columns) {
      fields.push({
        path: [c.name],
        type: c.type,
        jsType: _chunkFUSDVFROjs.convertCHDataTypeToJSType.call(void 0, c.type)
      });
    }
    const mapColumns = _nullishCoalesce(_chunkFUSDVFROjs.filterColumnMetaByType.call(void 0, columns, ["map" /* Map */]), () => ( []));
    await Promise.all(
      mapColumns.map(async (column) => {
        ...
        for (const key of keys) {
          fields.push({
            ...
            type: chType,
            jsType: _chunkFUSDVFROjs.convertCHDataTypeToJSType.call(void 0, chType)
          });
        }
      })
    );
  ...
  async getKeyValues({
    ...
    return this.cache.getOrFetch(
      ...
      async () => {
        const sql = await _chunkFUSDVFROjs.renderChartConfig.call(void 0, 
          {
            ...chartConfig,
