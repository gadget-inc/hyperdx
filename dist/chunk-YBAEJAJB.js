    ...
    return concatChSql(
      ...
      await Promise.all(
        withClauses.map(async (clause) => {
          ...
          if (sql && !_chunkE5P4KR33js.ChSqlSchema.safeParse(sql).success) {
            throw new Error("non-conforming sql object in CTE");
          }
          if (chartConfig2 && !_chunkE5P4KR33js.ChartConfigSchema.safeParse(chartConfig2).success) {
...
var SQLSerializer = (_class = class {constructor() { _class.prototype.__init.call(this);_class.prototype.__init2.call(this); }
  __init() {this.NOT_FOUND_QUERY = "(1 = 0)"}
  __init2() {this.caseSensitive = false}
  operator(op) {
    switch (op) {
      case "NOT":
      case "AND NOT":
  ...
  async fieldSearch(field, term, isNegatedField, prefixWildcard, suffixWildcard) {
    ...
    if (propertyType === "bool" /* Bool */) {
      ...
      return _sqlstring2.default.format(
        `(${_optionalChain([columnJSON, 'optionalAccess', _74 => _74.string])} ${isNegatedField ? "NOT " : ""}${this.caseSensitive ? "LIKE" : "ILIKE"} ?)`,
        [`%${term}%`]
      );
    }
    if (term.length === 0) {
    ...
    if (isImplicitField) {
      if (prefixWildcard || suffixWildcard) {
        return _sqlstring2.default.format(
          this.caseSensitive ? `(? ${isNegatedField ? "NOT " : ""}LIKE ?)` : `(lower(?) ${isNegatedField ? "NOT " : ""}LIKE lower(?))`,
          [
...
var CustomSchemaSQLSerializerV2 = class extends SQLSerializer {
  ...
  constructor({
    ...
    this.caseSensitive = _nullishCoalesce(caseSensitive, () => ( false));
  }
  /**
